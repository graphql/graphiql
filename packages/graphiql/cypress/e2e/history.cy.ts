...
describe('history', () => {
  ...
  it('will save if new query is different than previous query', () => {
    cy.visit(`?queryy=${mockQuery1}&headers=${mockHeaders1}`);
    cy.get('button[aria-label="Show History"]').click();
    cy.clickExecuteQuery();
    cy.get('ul.graphiql-history-items li').should('have.length', 1);

    cy.visit(`?queryy=${mockQuery2}&headers=${mockHeaders1}`);
    cy.clickExecuteQuery();
  ...
  it('will save query if the variables change', () => {
    cy.visit(
      `?queryy=${mockQuery1}&headers=${mockHeaders1}&defaultVariables=${mockVariables1}`,
    );
    cy.get('button[aria-label="Show History"]').click();
    cy.clickExecuteQuery();
    cy.get('ul.graphiql-history-items li').should('have.length', 1);
    ...
    cy.visit(
      `?queryy=${mockQuery1}&headers=${mockHeaders1}&defaultVariables=${mockVariables2}`,
    );
    cy.clickExecuteQuery();
    cy.get('ul.graphiql-history-items li').should('have.length', 2);
  });
  ...
  it('will save query if the headers change', () => {
    ...
    cy.visit(`?queryy=${mockQuery1}&headers=${mockHeaders2}`);
    cy.clickExecuteQuery();
    cy.get('ul.graphiql-history-items li').should('have.length', 2);
  });
