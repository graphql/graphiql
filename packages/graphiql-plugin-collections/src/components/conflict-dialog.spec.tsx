...
const savedCollectionsState = collectionsStore.getState().collections;

beforeEach(() => {
  collectionsStore.setState({ collections: [] });
});

afterEach(() => {
  collectionsStore.setState({ collections: savedCollectionsState });
});
...
describe('ConflictDialog', () => {
  ...
  describe('Review each…', () => {
    ...
    it('Back clears checked state and returns to action buttons', () => {
      renderDialog();
      // Enter review mode.
      fireEvent.click(screen.getByText('Review each…'));
      // Check the first checkbox (ChangedOp = id x1).
      const checkboxesBefore =
        screen.getAllByRole<HTMLInputElement>('checkbox');
      fireEvent.click(checkboxesBefore[0]!);
      expect(checkboxesBefore[0]!.checked).toBe(true);

      // Click Back — should return to the non-review action buttons.
      fireEvent.click(screen.getByText('Back'));
      expect(screen.getByText('Apply changes')).toBeDefined();
      expect(screen.getByText('Keep my versions')).toBeDefined();
      expect(screen.queryByText('Confirm selection')).toBeNull();

      // Enter review again — checkbox must be unchecked (state did not leak).
      fireEvent.click(screen.getByText('Review each…'));
      const checkboxesAfter = screen.getAllByRole<HTMLInputElement>('checkbox');
      checkboxesAfter.forEach(cb => expect(cb.checked).toBe(false));
    });
  });

  describe('"in {collectionName}" label', () => {
    it('shows the collection name next to a conflict row when the store has it', () => {
      // sampleAnalysis has changedItems with currentCollectionId === 'c1'.
      ...
      renderDialog();
      // Enter review mode so the ReviewRow renders.
      fireEvent.click(screen.getByText('Review each…'));

      // Both changed items have currentCollectionId === 'c1' which resolves to
      // "My Stuff". There are two ReviewRows so two "in My Stuff" spans.
      const labels = screen.getAllByText('in My Stuff');
      expect(labels.length).toBeGreaterThanOrEqual(1);
    });

    it('omits the "in …" label when the collection id is not in the store', () => {
      // Store is empty — no collections seeded.
      renderDialog();
      fireEvent.click(screen.getByText('Review each…'));
