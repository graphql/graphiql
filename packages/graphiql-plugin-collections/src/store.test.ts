...
describe('setCollections', () => {
  ...
  it('does not call storage.save', async () => {
    ...
    // Give any pending microtasks a chance to run; save must still be absent.
    await vi.waitFor(() => {
...
describe('persistence round-trip', () => {
  it('save then load restores data', async () => {
    ...
    // Wait until the async persist has written to storage.
    await vi.waitFor(() => {
...
describe('tab-link persistence', () => {
  it('linkTab persists to storage and init restores the map', async () => {
    const storage = makeStorage();
    await getActions().init(storage);
    const col = getActions().createCollection('C');
    const item = getActions().addItem(col.id, { name: 'Op', query: '{ a }' });
    getActions().linkTab('tab-1', col.id, item.id);

    // Simulate a reload: drop in-memory links, then re-init.
    collectionsStore.setState({ links: {} });
    await getActions().init(makeStorage());

    expect(collectionsStore.getState().links['tab-1']).toEqual({
      collectionId: col.id,
      itemId: item.id,
    });
  });

  it('⌘S updates in place using a link restored on init (reload scenario)', async () => {
    let persisted: Collection[] = [];
    const storage: CollectionsStorage = {
      async load() {
        return persisted;
    ...
    // Session 1: create, save (link), let it persist.
    await getActions().init(storage);
    const col = getActions().createCollection('C');
    const item = getActions().addItem(col.id, { name: 'Op', query: '{ old }' });
    getActions().linkTab('tab-1', col.id, item.id);
    ...
    // Session 2 (reload): fresh in-memory state, same persisted storage.
    collectionsStore.setState({ collections: [], loaded: false, links: {} });
    await getActions().init(storage);
  ...
  it('linkTab drops a stale link to the same item (one live tab per item)', async () => {
    await getActions().init(makeStorage());
    const col = getActions().createCollection('C');
    const item = getActions().addItem(col.id, { name: 'Op', query: '{ a }' });
    getActions().linkTab('tab-old', col.id, item.id);
    getActions().linkTab('tab-new', col.id, item.id);

    const { links } = collectionsStore.getState();
    expect(links['tab-old']).toBeUndefined();
    expect(links['tab-new']).toEqual({ collectionId: col.id, itemId: item.id });
  });

  it('deleteItem prunes its link and persists the pruned map', async () => {
    await getActions().init(makeStorage());
    const col = getActions().createCollection('C');
    const item = getActions().addItem(col.id, { name: 'Op', query: '{ a }' });
    getActions().linkTab('tab-1', col.id, item.id);
    getActions().deleteItem(col.id, item.id);

    expect(collectionsStore.getState().links['tab-1']).toBeUndefined();
    // Persisted: a reload sees no link either.
    collectionsStore.setState({
      links: { stale: { collectionId: 'x', itemId: 'y' } },
    });
    await getActions().init(makeStorage());
    expect(collectionsStore.getState().links).toEqual({});
  });

  it('deleteCollection prunes links for all of its items', async () => {
    await getActions().init(makeStorage());
    const col = getActions().createCollection('C');
    const item = getActions().addItem(col.id, { name: 'Op', query: '{ a }' });
    getActions().linkTab('tab-1', col.id, item.id);
    getActions().deleteCollection(col.id);
...
// ---------------------------------------------------------------------------
// Task 5: additional targeted tests for the store-layer fixes
// ---------------------------------------------------------------------------

describe('analyzeImport / applyImport — stray empty collection fix', () => {
  it('changed item whose incoming parent is new locally does NOT create an empty collection', async () => {
    const storage = makeStorage();
    await getActions().init(storage);

    // Local setup: col-A exists with item-1 inside it.
    const colA = getActions().createCollection('A');
    const item = getActions().addItem(colA.id, {
      name: 'Op',
      query: '{ original }',
    });

    // Import: item-1 appears under a brand-new collection id "col-B" with changed content.
    const incomingColB = makeCol('col-B', 'B', [
      makeItem({ id: item.id, name: 'Op', query: '{ changed }' }),
    ]);
    const analysis = getActions().analyzeImport(
      JSON.stringify({ version: 1, collections: [incomingColB] }),
    );

    // Should be a changed item, not new.
    expect(analysis.changedItems).toHaveLength(1);
    expect(analysis.changedItems[0]?.currentCollectionId).toBe(colA.id);
    // No empty phantom collection shell for col-B.
    expect(analysis.newCollections).toHaveLength(0);

    const colCountBefore = collectionsStore.getState().collections.length;
    getActions().applyImport(analysis, { mode: 'merge', applyChanges: true });

    const state = collectionsStore.getState().collections;
    // Collection count must not have grown.
    expect(state).toHaveLength(colCountBefore);
    // Item is updated in its current collection (col-A).
    const aItems = state.find(c => c.id === colA.id)?.items ?? [];
    expect(aItems[0]?.query).toBe('{ changed }');
    // col-B was never created.
    expect(state.find(c => c.id === 'col-B')).toBeUndefined();
  });

  it('new collection that carries genuinely new items IS created', () => {
    const newItem = makeItem({
      id: 'item-new',
      name: 'Fresh',
      query: '{ fresh }',
    });
    const newCol = makeCol('col-new', 'New Collection', [newItem]);
    const analysis = getActions().analyzeImport(
      JSON.stringify({ version: 1, collections: [newCol] }),
    );

    expect(analysis.newCollections).toHaveLength(1);
    expect(analysis.newCollections[0]?.id).toBe('col-new');
    expect(analysis.newItems).toHaveLength(1);
    ...
    expect(created).toBeDefined();
    expect(created?.items).toHaveLength(1);
    expect(created?.items[0]?.id).toBe('item-new');
  });

  it('intentionally empty new collection is created as an empty shell', () => {
    const emptyCol = makeCol('col-empty', 'Empty', []);
    const analysis = getActions().analyzeImport(
      JSON.stringify({ version: 1, collections: [emptyCol] }),
    );

    expect(analysis.newCollections).toHaveLength(1);
    expect(analysis.newCollections[0]?.id).toBe('col-empty');
    expect(analysis.newItems).toHaveLength(0);

    getActions().applyImport(analysis, { mode: 'merge', applyChanges: true });

    const created = collectionsStore
      .getState()
      .collections.find(c => c.id === 'col-empty');
    expect(created).toBeDefined();
    expect(created?.items).toHaveLength(0);
  });
});

describe('analyzeImport — zod validation', () => {
  it('returns ok:false for a malformed envelope (item missing required id)', async () => {
    const storage = makeStorage();
    await getActions().init(storage);
    const colA = getActions().createCollection('A');
    const before = collectionsStore.getState().collections;
      ...
      collections: [
        {
          ...
          createdAt: 1000,
          updatedAt: 1000,
          // items contains an object missing the required "id" field
          items: [
            { name: 'Op', query: '{ q }', createdAt: 1000, updatedAt: 1000 },
          ],
        },
      ],
    });

    const analysis = getActions().analyzeImport(bad);
    expect(analysis.ok).toBe(false);

    // applyImport of an ok:false analysis is a no-op.
    getActions().applyImport(analysis, { mode: 'merge', applyChanges: true });
    expect(collectionsStore.getState().collections).toHaveLength(before.length);
    expect(collectionsStore.getState().collections[0]?.id).toBe(colA.id);
  });

  it('returns ok:false when collections contains a non-object entry', () => {
    const bad = JSON.stringify({
      version: 1,
      collections: [42, 'not-a-collection'],
    });
    const analysis = getActions().analyzeImport(bad);
    expect(analysis.ok).toBe(false);
  });

  it('returns ok:false when a required number field (createdAt) is a string', () => {
    const bad = JSON.stringify({
      version: 1,
      collections: [
        {
...
describe('analyzeImport — method removal idempotent round-trip', () => {
  it('re-importing an unchanged export returns unchangedCount matching item count', async () => {
    const storage = makeStorage();
    await getActions().init(storage);
    const col = getActions().createCollection('Round-trip');
    getActions().addItem(col.id, { name: 'A', query: '{ a }' });
    getActions().addItem(col.id, { name: 'B', query: '{ b }' });
