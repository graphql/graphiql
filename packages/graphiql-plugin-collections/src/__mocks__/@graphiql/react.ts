...
export const __state = {
  ...
  // When null, a single default tab derived from `queryText` is used.
  tabs: null as MockTab[] | null,
  activeTabIndex: 0,
  addTab() {},
  changeTab(_index: number) {},
...
function currentTabs(): MockTab[] {
  return __state.tabs ?? [{ id: 'tab-0', query: __state.queryText }];
}
...
export function useGraphiQL<T>(selector: Selector<T>): T {
  return selector({
    ...
    queryEditor: null,
    activeTabIndex: __state.activeTabIndex,
    tabs: currentTabs(),
  });
}
