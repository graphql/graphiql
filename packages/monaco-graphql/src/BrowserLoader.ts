...
export class BrowserLoader implements DocumentLoader {
    ...
    async canLoad(_pointer: SchemaPointerSingle, options: BrowserLoaderOptions): Promise<boolean> {
        return Boolean(storage.get(options.path))
    }

    async load(pointer: SchemaPointerSingle, options: BrowserLoaderOptions): Promise<Source> {
        if (!options.path) {
        ...
        try {
            ...
            if (!jsonContent) {
                await storage.set(options.path, `{ "schema": "http://localhost:8080/graphql" }`)
            }
            return parseGraphQLJSON(pointer, jsonContent, options);
        } catch (e) {
            throw new Error(`Unable to read browser json: ${options.path}: ${e.message || e}`);
        }
    }
}
